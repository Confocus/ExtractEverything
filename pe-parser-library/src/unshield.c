...
#else
#include "libunshield.h"//todo：统一路径
#endif
#ifdef HAVE_CONFIG_H
#ifdef _WIN32
#include "lib/unshield_config.h"
#else
#include "unshield_config.h"//todo：统一路径
#endif
#endif
...
static bool make_sure_directory_exists(const char* directory)/*{{{*/
{
  ...
  //这里是一级一级创建目录，临时屏蔽掉，外部调用时保证目录存在即可
#ifdef _WIN32
  _mkdir(directory);
#else
...
void unshield_set_output_directory(const char* const path)
{
...
int unshield_extract_helper(Unshield* unshield, const char* prefix, int first, int last)/*{{{*/
{
  int i;
  int count = 0;
...
bool unshield_do_action(Unshield* unshield, ActionHelper helper)
{
  int count = 0;
...
int main2(int argc, char* const argv[])
{
  ...
  switch (action)
  {
    ...
    case ACTION_LIST_FILES:
      success = unshield_do_action(unshield, list_files_helper);
      break;
    ...
    case ACTION_TEST:
      ...
      success = unshield_do_action(unshield, test_helper);
      break;
  }
