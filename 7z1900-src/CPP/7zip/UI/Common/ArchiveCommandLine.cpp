    ...
    CArcCmdLineOptions &options)
{
  ...
  options.IsInTerminal = MY_IS_TERMINAL(stdin);//1
  options.IsStdOutTerminal = MY_IS_TERMINAL(stdout);//1
  options.IsStdErrTerminal = MY_IS_TERMINAL(stderr);//1

  options.HelpMode = parser[NKey::kHelp1].ThereIs || parser[NKey::kHelp2].ThereIs  || parser[NKey::kHelp3].ThereIs;//0

  options.StdInMode = parser[NKey::kStdIn].ThereIs;//0
  options.StdOutMode = parser[NKey::kStdOut].ThereIs;//0
  options.EnableHeaders = !parser[NKey::kDisableHeaders].ThereIs;//1
  options.TechMode = parser[NKey::kTechMode].ThereIs;//0
  options.ShowTime = parser[NKey::kShowTime].ThereIs;//0

  if (parser[NKey::kDisablePercents].ThereIs
      || options.StdOutMode
      || !options.IsStdOutTerminal)
    options.Number_for_Percents = k_OutStream_disabled;//1

  if (options.StdOutMode)
    options.Number_for_Out = k_OutStream_disabled;//1

  SetStreamMode(parser[NKey::kOutStream], options.Number_for_Out);//未执行
  SetStreamMode(parser[NKey::kErrStream], options.Number_for_Errors);//未执行
  SetStreamMode(parser[NKey::kPercentStream], options.Number_for_Percents);//未执行

  if (parser[NKey::kLogLevel].ThereIs)//未执行
  {
  ...
  if (parser[NKey::kCaseSensitive].ThereIs)//未执行
  {
  ...
  if (parser[NKey::kLargePages].ThereIs)//未执行
  {
    unsigned slp = 0;
  ...
  if (parser[NKey::kAffinity].ThereIs)//未执行
  {
...
void CArcCmdLineParser::Parse2(CArcCmdLineOptions &options)
{
  ...
  if (!ParseArchiveCommand(nonSwitchStrings[kCommandIndex], options.Command))//判断命令行参数"l"在"audtexlbih"中的第几个，返回这个数值到options.Command
    throw CArcCmdLineException("Unsupported command:", nonSwitchStrings[kCommandIndex]);

  if (parser[NKey::kHash].ThereIs)//0
    options.HashMethods = parser[NKey::kHash].PostStrings;
  
  if (parser[NKey::kElimDup].ThereIs)//0
  {
  ...
  if (parser[NKey::kNameTrailReplace].ThereIs)//0
  ...
  if (parser[NKey::kRecursed].ThereIs)
    ...
    recursedType = NRecursedType::kNonRecursed;//1

  bool wildcardMatching = true;
  if (parser[NKey::kDisableWildcardParsing].ThereIs)//0
  ...
  if (parser[NKey::kInclude].ThereIs)//0
  {
  ...
  if (parser[NKey::kExclude].ThereIs)//0
    AddSwitchWildcardsToCensor(options.Censor,
  ...
  bool thereIsArchiveName = !parser[NKey::kNoArName].ThereIs &&
      ...
      options.Command.CommandType != NCommandType::kHash;//1

  bool isExtractGroupCommand = options.Command.IsFromExtractGroup();//0
  bool isExtractOrList = isExtractGroupCommand || options.Command.CommandType == NCommandType::kList;//1
  bool isRename = options.Command.CommandType == NCommandType::kRename;//0
