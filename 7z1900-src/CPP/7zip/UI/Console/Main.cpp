...
)
{
  ...
  //{
  #ifdef _WIN32
  NCommandLineParser::SplitCommandLine(GetCommandLineW(), commandStrings);//分割命令行,目标路径保存到了NonSwitchStrings中
  #else
...
//}
  ...
  if (options.Number_for_Out != k_OutStream_stdout)//未执行
    g_StdStream = (options.Number_for_Out == k_OutStream_stderr ? &g_StdErr : NULL);

  if (options.Number_for_Errors != k_OutStream_stderr)//未执行
  ...
  if (percentsStream)
  {
    ...
    if (GetConsoleScreenBufferInfo(GetStdHandle(STD_OUTPUT_HANDLE), &consoleInfo))//用于检索指定的控制台屏幕缓冲区的信息
  ...
  bool isExtractGroupCommand = options.Command.IsFromExtractGroup();//0

  if (codecs->Formats.Size() == 0 &&//0
  ...
  FOR_VECTOR (k, options.ExcludedArcTypes)//0
  {
...
#ifdef _WIN32
extern "C"  int _cdecl UnpackBy7z(int numArgs, char *args[])
#else
extern "C"  int UnpackBy7z(int numArgs, char *args[])
#endif
{
	//添加用来在调用接口时输出内容
	...
	for (uint32_t i = 0; i < numArgs; i++)
	{
		...
		UString UStr;
		UStr = AStr;
		commandStrings.Add(UStr);
	}
#else
	//7z16.02版本中要使用这段代码
	for (int i = 0; i < numArgs; i++)
	{
		wchar_t szArgv[0x100] = { 0 };
		mbstowcs(szArgv, args[i], strlen(args[i]));
		UString UStr;
		UStr = szArgv;
		commandStrings.Add(UStr);
	}
...
//7z16.02
int IsPacked(const char* szFilePath)
{
	bool bIsPE = false;
	...
	options.stream = NULL;
#ifdef _WIN32
	int nLen = MultiByteToWideChar(CP_ACP, 0, szFilePath, -1, NULL, NULL);
	MultiByteToWideChar(CP_ACP, 0, szFilePath, -1, wszFilePath, nLen);
#else
	mbstowcs(wszFilePath, szFilePath, strlen(szFilePath));
#endif
	options.filePath = wszFilePath;

	CArchiveLink arcLink;//这个的顺序一定要在CREATE_CODECS_OBJECT后边
#ifdef _WIN32
	if (result = arcLink.Open_Strict(options, &openCallback) != S_OK)
#else
	if (result = arcLink.Open3(options, &openCallback) != S_OK)
#endif
	{
		return false;
	}

	if (1 == arcLink.Arcs.Size())
	{
		if (StringsAreEqualNoCase_Ascii(codecs->Formats[arcLink.Arcs[0].FormatIndex].Name, "pe"))
		{
			//printf("Summary: It's a PE that cann't be unpacked by 7zO..\n");
			//如果7z判断出来是PE，仍要走后边的判断其他壳类型的逻辑
			//printf("PE\n");
			return false;
		}
		else
		{
			//printf("%ls\n", codecs->GetFormatNamePtr(arcLink.Arcs[0].FormatIndex));
			//printf("7z_check\n", codecs->GetFormatNamePtr(arcLink.Arcs[0].FormatIndex));
			return true;
		}
	}

	for (unsigned r = 1; r < arcLink.Arcs.Size(); r++)//Nsis/7z/Cab/zip/rar
	{
		if (StringsAreEqualNoCase_Ascii(codecs->Formats[arcLink.Arcs[r].FormatIndex].Name, "nsis") ||
			StringsAreEqualNoCase_Ascii(codecs->Formats[arcLink.Arcs[r].FormatIndex].Name, "7z")   ||
			StringsAreEqualNoCase_Ascii(codecs->Formats[arcLink.Arcs[r].FormatIndex].Name, "cab")  ||
			StringsAreEqualNoCase_Ascii(codecs->Formats[arcLink.Arcs[r].FormatIndex].Name, "zip")  ||
			StringsAreEqualNoCase_Ascii(codecs->Formats[arcLink.Arcs[r].FormatIndex].Name, "rar"))
		{
			//printf("ls\n", codecs->GetFormatNamePtr(arcLink.Arcs[r].FormatIndex));
			//printf("7z_check\n", codecs->GetFormatNamePtr(arcLink.Arcs[r].FormatIndex));
			return true;
		}
	}
